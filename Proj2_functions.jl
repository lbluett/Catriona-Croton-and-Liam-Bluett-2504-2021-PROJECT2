...
next_service_time(s::State, q::Int) = rand(rate_scv_gamma(s.params.μ_array[q], s.params.gamma_scv))
...
routing_function(q::Int, P::Matrix{Float64}) = sample(1:L+1, weights(push!(P[q, :], 1 - sum(P[q, :]))))

# function to return which queue to join or if leave when overflow, according to Q matrix
overflow_function(q::Int, Q::Matrix{Float64}) = sample(1:L+1, weights(push!(Q[q, :], 1 - sum(Q[q, :]))))
