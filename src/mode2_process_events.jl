# Process events for Mode 2 of the simulation
...
"""
    ...
    Returns the next external arrival and a Mode2QueueArrivalEvent to arrive at a queue according to vector p_e
"""
function process_event(time::Float64, state::State, park_arrival_event::Mode2ExternalArrivalEvent)
    ...
    push!(new_timed_events,TimedEvent(Mode2ExternalArrivalEvent(), time + next_park_arrival_time(state))) # create next arrival event
    ...
    push!(new_timed_events,TimedEvent(Mode2QueueArrivalEvent(q, state.total_entered), time)) # create queue arrival event
...
"""
    ...
    If is the only job in the queue, then engage service and create a Mode2EndOfServiceAtQueueEvent
"""
function process_event(time::Float64, state::State, queue_arrival_event::Mode2QueueArrivalEvent)
    ...
    if length(state.queues[q]) == 0
        ...
        push!(new_timed_events, TimedEvent(Mode2EndOfServiceAtQueueEvent(q, queue_arrival_event.id, 0), time + next_service_time(state,q)))
    ...
    elseif state.params.K[q] > 0 && length(state.queues[q]) == state.params.K[q]
        push!(new_timed_events, TimedEvent(Mode2OverflowEvent(q, queue_arrival_event.id), time))
...
function process_event(time::Float64, state::State, eos_event::Mode2EndOfServiceAtQueueEvent)
    ...
    if length(state.queues[q]) ≥ 1
        push!(new_timed_events, TimedEvent(Mode2EndOfServiceAtQueueEvent(q, dequeue!(state.queues[q]), 1), time + next_service_time(state, q)))
        #state.service += 1
    ...
    if q_move_to <= state.params.L
        push!(new_timed_events, TimedEvent(Mode2QueueArrivalEvent(q_move_to, eos_event.id), time + moving_time(state)))
        #state.move += 1
    # otherwise if not chosen a queue (if L+1), then leave
    else
        push!(new_timed_events, TimedEvent(Mode2LeaveParkEvent(eos_event.id), time))
    end
...
"""
function process_event(time::Float64, state::State, overflow_event::Mode2OverflowEvent)
    ...
    # if moving to a queue, then create new event that is when arrives at the queue and add to the moving counter
    if q_move_to <= state.params.L
        push!(new_timed_events, TimedEvent(Mode2QueueArrivalEvent(q_move_to, overflow_event.id), time + moving_time(state)))
        state.move += 1
    # otherwise if not chosen a queue (if L+1), then leave
    else
        push!(new_timed_events, TimedEvent(Mode2LeaveParkEvent(overflow_event.id), time))
    end
...
"""
    LeaveParkEvent()
"""
function process_event(time::Float64, state::State, leave_event::Mode2LeaveParkEvent)
    new_timed_events = TimedEvent[]
