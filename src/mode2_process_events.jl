...
function process_event(time::Float64, state::State, queue_arrival_event::QueueArrivalEvent)
    ...
    # if server isn't busy, then make server busy and add a EndOfServiceAtQueueEvent
    if !state.serverBusy[q] 
        state.serverBusy[q] = true
        push!(new_timed_events, TimedEvent(EndOfServiceAtQueueEvent(q, queue_arrival_event.id), time + next_service_time(state,q)))
    ...
    #= previous code not using servers
    enqueue!(state.queues[q], queue_arrival_event.id)
...
function process_event(time::Float64, state::State, eos_event::EndOfServiceAtQueueEvent)
    ...
    # if no one waiting in this queue, then set up server not busy
    if length(state.queues[q]) == 0
        state.serverBusy[q] == false
    # otherwise dequeue and start new EndOfServiceAtQueueEvent
    else
        id = dequeue!(state.queues[q])
        push!(new_timed_events, TimedEvent(EndOfServiceAtQueueEvent(q, id), time + next_service_time(state, q)))
    end

    # determine where the next queue is for the id finishing
